...
export const startListeningWithTimer = (timerRef, recognitionRef, setStatus, setCharInput, currentChar, speechInputType, setDisplayedChar) => {
    console.log("start");
    try {
        ...
        recognition.onresult = (event) => {
            ...
            if (currentChar) {
                ...
                verifyAndSetChar(spokenInput, confidence, currentChar, setStatus, setCharInput, speechInputType);
            } else {
                console.log("set input only")
                setInputOnly(spokenInput, confidence, setStatus, setCharInput, speechInputType, setDisplayedChar)
            }
            recognition.stop();
        }
...
export const verifyAndSetChar = async (input, confidence, currentChar, setStatus, setCharInput, speechInputType) => {
    console.log('current', currentChar);
    console.log('input', input);
    console.log('confidence', confidence);

    // Practice only supports letters for now
    if (speechInputType === 'letter') {
        const extractedLetter = extractLetterFromInput(input, confidence);
        if (extractedLetter) {
            if (extractedLetter === currentChar) {
                setCharInput(currentChar);
                setStatus(states.correct);
                return;
            } else {
                setCharInput(extractedLetter);
...
export const setInputOnly = async (input, confidence, setStatus, setCharInput, speechInputType, setDisplayedChar) => {
    console.log('input', input);
    console.log('confidence', confidence);
    ...
    if (speechInputType === 'letter') {
        ...
        if (extractedLetter) {
            ...
            setCharInput(extractedLetter);
            setDisplayedChar(extractedLetter);
...
const extractLetterFromInput = (input, confidence) => {
    if (input.trim().length === 1) {
    ...
    if (input.startsWith("letter ") && input.length > 7 && confidence >= 0.5) {
        const detectedLetter = input.split(" ")[1];
        if (detectedLetter && detectedLetter.length === 1) {
            return detectedLetter.trim().toLowerCase();
        }
    }
